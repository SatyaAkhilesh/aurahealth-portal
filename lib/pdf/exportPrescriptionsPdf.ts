  ...
  const rows = prescriptions
    .map((p) => {
      const isRefillSoon =
        ...
        (() => {
          const diff =
            (new Date(p.refill_on).getTime() - new Date().getTime()) /
            (1000 * 60 * 60 * 24)
      ...
      return `
        <tr>
          ...
          <td>${p.dosage || '—'}</td>
          <td>${p.quantity ?? '—'}</td>
          <td style="${isRefillSoon ? 'color:#D97706;font-weight:bold;' : ''}">
            ${p.refill_on || '—'}${isRefillSoon ? ' ⚠️' : ''}
          </td>
          ...
          <table>
            ...
            <tbody>
              ${
                rows.length > 0
                  ? rows
                  : '<tr><td colspan="6" style="text-align:center;color:#64748B;padding:24px;">No prescriptions on record.</td></tr>'
              }
            </tbody>
          </table>
          <div class="footer">
